...
Следующие функции составляют командный интерфейс "робота", с помощью которго (и только с помощью его) с ним можно взаимодействовать.
...
По использованию любой из них можно получить исчерпывающую информацию, используя встроенную систему помощи (но только после того как будет выполено include("robot.jl"), и перейдя для этого из стандартного режима REPL, в режим помощи, см. выше). 
...
Во всех приведенных примерах функций типы их аргументы были аннотированы. Однако в языке Julia такая аннотация не является обязательной, т.е. можно было бы и не указывать типы аргументов функций, так же как это, например, имеет место в языке Python. Поэтому, если во всех приведенных примерах функций, убрать аннотацию типов аргументов, то по-прежнему все будет работать. 
...
## Краткая справка по использованным в примере синтаксическим конструкциям языка Julia

1. Определение функции может иметь следующие варианты:
...
Слово function может и отсутствовать в определении функции:

    имя_функции(список_аргументов_функции)=выражение_определяющее_функцию

Кроме того, во всех вариантах определения функции после скобок со списком параметров (который может быть и пустым) можно, при желании, указывать тип возвращаемого функцией значения. Например, в определении

    move_if_possible!(r::Robot,side::HorizonSide)::Bool=...

указано, что функция возвращает логическое значение.

2. Конструкция (HorizonSide(i) for i=0:3) представляет собой частный случай генератора последовательности. Т.е. если бы было [HorizonSide(i) for i=0:3], то это
была бы операция "спискового включения" (как в Python), результатом которой был бы вектор (массив) из 4-х элементов. А в нашем случае (HorizonSide(i) for i=0:3) массив не формируется, а генерируется последовательность значений, которые входили бы в списковое включение, но не сразу, а по мере надобности. Таким образом, при использовании генератора вместо массива дает экономию памяти (в данном случае не существенную ввиду малой длины последовательности).

3. "Арифметический" цикл имеет вид:
    for значение in итерируемый_объект
       тело_цикла
    end

4. Цикл с предусловием имеет вид:
...
6. Оператор ветвления в Julia имеет вид:
    if условие
        тело_секции_if
    else
        тело_секции_else
...
Возможны также такие укороченный вариан:
...
В любом случае, значением оператора if будет значение последнего оператора в теле фактически исполняемой секции.
...
Обычно также в классически ООП языках используется точечная нотации для вызова какого-либо метода для данного объекта. Например, команда сделать шаг в направлении Nord в языке Python могла бы выглядеть так: r.move(Nord). Но в языке Julia точечная нотация для вызова функций, ассоциированных с данным типом не используется. Объекты соответствующего типа могут являться лишь обычными агргументами соответствующей функции.