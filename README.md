...
А вот в языке Julia вместо классов есть типы, и у типов вместо методов есть функции, а методы, в свою очередь, есть только у функций (см. выше). При этом принято говорить, что тот или иной конкретный метод СПЕЦИАЛИЗИРУЕТ обобщенную (generic) функцию.